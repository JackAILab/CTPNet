...
class CTPnet(nn.Module):
    def __init__(self, recurrent_iter=6, use_GPU=True, factor=10e-3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        # 6~8层要先上采样再卷积 # 直接上采样
        self.conv6 = Basic(512+512, 512, channel_att=False, spatial_att=False)
        self.conv7 = Basic(256+512, 256, channel_att=False, spatial_att=False)
        self.conv8 = Basic(256+128, out_channel, channel_att=False, spatial_att=False)
        ...
        self.conv0 = nn.Sequential(
            nn.Conv2d(15, 32, 3, 1, 1), # pre1 3channels===pre2 3channels====x 3channels====input 3channels   3*4=12 channels
            nn.ReLU()
        )
        self.res_conv1 = nn.Sequential(
            nn.Conv2d(32, 32, 3, 1, 1),
    ...
    def forward(self, input):
        ...
        for i in range(3):
            if i==0: # 基于空间注意力的上采样
                ...
                conv6 = self.conv6(torch.cat([conv4, F.interpolate(conv5, scale_factor=2, mode=self.upMode)], dim=1)) # self.upMode为bilinear
                ...
                # return channel K*K*N
                core3 = self.outc(F.interpolate(conv8, scale_factor=2, mode=self.upMode)) # 恢复到原来的图像大小
        ...
        for i in range(self.iteration):  #　以下逐级扩张
            if i==0: # 第一次进行扩张
                ...
                # x_list.append(x)   
                derain=x     
            elif rain_judge(derain,rain=input, factor=self.factor)=="NotClear": # 不同于PreNet的是，AMCC2针对扩张次数进行了限定
                x = torch.cat((input3, x), 1)
                x = self.conv0(x)
                resx = x
                x = F.relu(self.res_conv1(x) + resx)
